// Switch-person nav target (BUG-007). Single source of truth for "go back to
...
//
// The picker is a login gate, not a browse ancestor, so this is deliberately
// separate from core/breadcrumb.js (Home stays the breadcrumb root). Returning
// to the picker re-runs the gate, so an adult person re-requires its PIN — there
// is no path that silently re-enters an adult person (FEAT-026 TASK-156).
