...
// TASK-276 — the audio player no longer resumes mid-song: a track always loads
// at 0. TASK-504 hardens that by construction — the unified queue engine
// carries no position in its snapshot at all (and registers no `position`
// action), exactly like the film, home-movie and music-video engines. This
// still guards the load path: a track change must not seek.
...
// np.position → a write; it passes 0 now → no write.
test('a track change loads at 0 — no resume seek (TASK-276)', async ({ page }) => {
  ...
  // Server now-playing becomes a DIFFERENT track (ootb-02); the resulting
  // broadcast reaches the page as a track change — the load path swapTrack
  // would seek on, if it still seeked.
  await page.evaluate(() => fetch('/api/queue/music/play-item?person=kids', {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ item_id: 'ootb-02' })
  }));
