...
// TASK-258 (1) regression — the test above uses the desktop viewport + the short
// 'TV' label, which fit on one row even back when #bar-row was allowed to wrap. The
// real bug shows at a COMPANION (phone) width with a realistic long screen name: a
// wrapping row dropped the Control/Browse toggle onto its own line under the pill.
// The pill must shrink + ellipsize so the two bars stay on one row (nowrap). A
// dedicated mock supplies a long device label + a phone viewport.
test.describe('one-row header at phone width, long screen name (TASK-258 regression)', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.routeWebSocket(/:8766/, (ws) => {
      ws.onMessage(function(raw) {
        const m = JSON.parse(raw);
        if (m.type === 'list_devices') ws.send(msg('devices', { devices: [{ device_id: 'tv', label: 'Living Room Television', active_person: null }] }));
        if (m.type === 'snapshot_request') { ws.send(msg('context', { version: 2, context_id: 'browse' })); ws.send(msg('app_state', { screen: 'home', profile: 'kids' })); }
      });
    });
    await page.goto('/companion/browse.html');
    await expect(page.locator('#sections-row .chip').first()).toBeVisible();
  ...
  test('a long screen name does not stack the toggle below the pill', async ({ page }) => {
    await expect(page.locator('#screen-bar .screen-current')).toBeVisible();
    const screenBox = await page.locator('#screen-bar').boundingBox();
    const syncBox = await page.locator('#sync-bar').boundingBox();
    // Same row: the two boxes overlap vertically (a wrapped toggle would sit well below).
    expect(screenBox.y).toBeLessThan(syncBox.y + syncBox.height);
    expect(syncBox.y).toBeLessThan(screenBox.y + screenBox.height);
    // Toggle is right of the pill, not beneath it.
    expect(syncBox.x).toBeGreaterThan(screenBox.x);
    // And the toggle is fully on-screen (not shoved off the right edge).
    expect(syncBox.x + syncBox.width).toBeLessThanOrEqual(391);
  });
});
