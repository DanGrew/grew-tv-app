...
- `companion-status-menu.js` — the header popout menu (TASK-412, `companion/browse.html` only): opens/closes `#status-menu` off a single `#btn-status` icon, never on an outside tap. Consolidates Mode (`companion-sync-bar.js`), Screen (`companion-screen-bar.js`), Profile (`#switch-profile`) and Atlas (`#door`) into one menu by relocating their existing container elements in the HTML — those mounts are unchanged. Not yet wired on the other 8 companion pages that render the same four bars (a follow-up task).
