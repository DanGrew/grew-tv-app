...
- `music-video-playthrough.js` — client-owned playthrough for a single music video, a music-video playlist, an artist's music videos, or the whole-catalog Play All (TASK-445): order + index + Shuffle/Repeat, no server engine, no resume (TASK-374/407). Its `entryMode` is the one shared video.html dispatch table screen-video-page.js reads — TASK-446's `homeMoviesAll` mode routes through it too, but is SERVER-authoritative (the video engine's own source), not client-owned like the mv* modes it sits beside
