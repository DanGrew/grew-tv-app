...
- `browse-continue.js` — what BROWSE knows about a media type, as DATA (TASK-501): the four Continue buttons (id, label, order), the `{ page, params }` each press resolves to, and the one wording both surfaces render. Continue itself is the queue engine's own advance — `next` on that media type, fired by the player's continue entry — so nothing here does queue maths, and whether a button is live is `queue-shell-view.js`'s `transportState().next`, the same rule ⏭ uses. A fifth media type is an entry here
