...
- `screen-queue.js` / `screen-video-queue.js` — Queue View overlays (FEAT-031/040); `screen-video-queue.js` is the pre-FEAT-497 video one, ⚠️ unreachable since TASK-517 (no `engineMode` resolves to `video` any more), kept until the old engines are retired
