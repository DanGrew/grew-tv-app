...
- `queue-tabs.js` / `queue-crumb.js` — the Queue/Next/Coming-Up tab shell and the Queue crumb (FEAT-031/039); `queue-crumb.js` also carries the companion's own "‹ Now Playing › Queue" crumb (TASK-515), which the phone Queue pages hardcoded as a bare back button with no leaf. `queue-tabs.js` now serves the shell alone (`qsTabShellHtml`/`phTabShellHtml`); its legacy TV `tabShellHtml` has one consumer left, the unread `video-queue-view.js`, and goes with it in TASK-525
- `queue-shell-view.js` — **THE** FEAT-497 Queue UX shell (TASK-515): one implementation of `docs/QUEUE-UX-SHELL.md` for every media type, replacing the near-identical per-cutover copies below. Both surfaces come out of the same builders (only a class map differs), so TV and companion cannot drift; `transportState` is the ONE transport rule (⏭ live whenever anything is ahead, from the override queue or the source; ⏮/Shuffle/Repeat disabled-but-visible with no source, never hidden) — and TASK-517 made it the rule at the PLAYER's own control row and the companion's too, not just the Queue hero, which is what BUG-510/512 were. All four media types run entirely on it — home movies (TASK-516), films (TASK-517), music (TASK-504) and music videos (TASK-505), the last cutover, after which a per-type difference is a data entry here and never a branch
- `queue-shell-config.js` — what the shell knows about a media type, as DATA (TASK-515): media noun, fallback glyph, hero source-line resolver, row sub-line resolver, transport rule, and the ＋Queue map entry. `queueAdd()` is THE ＋Queue producer — one call replacing the per-screen dispatch tables that left home movies posting to a retired engine; TASK-505 collapsed its own per-engine routing too, since every ＋ press now appends to the unified queue under its media type. A fifth media type is an entry here, not new code
- `video-queue-view.js` — video Queue View model + markup (FEAT-040). ⚠️ The LAST per-media-type Queue view left, and unread — TASK-505 retired the music-video one, so every media type renders from `queue-shell-view.js` now; this ships until the legacy Queue leaves the tree (TASK-525)
- `video-player-router.js` — persistent video-player view-router (FEAT-037) for the OLD video engine. ⚠️ Read only by the companion's own legacy branch since TASK-517 moved its last TV consumer (the browse "Play Queue" entry) onto the unified engine — kept, like the engine itself, until the old engines are retired wholesale (TASK-525)
- `queue-playback-router.js` — **THE** view-router for the TASK-498 unified queue engine's own `queue_playback` snapshot (item_id-keyed now_playing, `queue`/`next`/`coming_up` lookahead lists), over `video-player-router.js` — every media type reads it since TASK-505 cut the last one (music videos) over: home movies (TASK-499/516), films (TASK-503, plus the browse "Play Queue" entry in TASK-517), music (TASK-504) and music videos. `queueCount` is browse's 🎬 pill count, moved here off the old engine's router in TASK-517; `isStaleResync` is the BUG-521 entry-time guard every cut-over rail applies
- `music-video-playthrough.js` — music-video ROUTING only: `entryMode` is the one shared video.html dispatch table screen-video-page.js reads (every mode routes through it), plus `playlistTrackTarget`/`playlistQueueKey` (TASK-374/376/421). Its two other halves are gone — BUG-485 retired the client-owned order/index playthrough, TASK-505 the `mvTransportVisibility` show/hide gate that paired with it (the shell's one `transportState` decides now)
...
- `companion-video-queue.js` — the pre-FEAT-497 video Queue View mirror, ⚠️ unreachable and kept until TASK-525. Music's own `companion-queue.js` went with TASK-504 and the music-video one with TASK-505 — `companion/music-queue.html` and `companion/music-video-queue.html` both mount `companion-queue-shell.js`
