...
- `queue-view.js` / `queue-tabs.js` / `queue-crumb.js` — music Queue View (FEAT-031/039); `queue-crumb.js` also carries the companion's own "‹ Now Playing › Queue" crumb (TASK-515), which the phone Queue pages hardcoded as a bare back button with no leaf
- `queue-shell-view.js` — **THE** FEAT-497 Queue UX shell (TASK-515): one implementation of `docs/QUEUE-UX-SHELL.md` for every media type, replacing the near-identical per-cutover copies below. Both surfaces come out of the same builders (only a class map differs), so TV and companion cannot drift; `transportState` is the ONE transport rule (⏭ live whenever anything is ahead, from the override queue or the source; ⏮/Shuffle/Repeat disabled-but-visible with no source, never hidden). Nothing points at it until TASK-516/517
- `queue-shell-config.js` — what the shell knows about a media type, as DATA (TASK-515): media noun, fallback glyph, hero source-line resolver, row sub-line resolver, transport rule, and the ＋Queue map entry. `queueAdd()` is THE ＋Queue producer — one routing table replacing the per-screen dispatch tables that left home movies posting to a retired engine. A fifth media type is an entry here, not new code
...
- `home-movies-queue-view.js` — ⚠️ superseded by `queue-shell-view.js` above; deleted when TASK-516 points home movies at the shell. The FEAT-497 Queue UX shell's first cutover (TASK-499): hero + Queue/Next/Coming-Up model + markup (TV `.qs-*` classes, companion reusing the existing `.ph-*` row/tab classes) over the TASK-498 unified queue engine's own `queue_playback` snapshot; every row plays via `play-item` on tap (never mutates queue/source), Coming Up rows are read-only (the engine itself refuses edits against `next_permutation`)
- `film-queue-view.js` — ⚠️ superseded by `queue-shell-view.js` above; deleted when TASK-517 points films at the shell. The FEAT-497 Queue UX shell's second cutover (TASK-503): same shape as `home-movies-queue-view.js` (hero + Queue/Next/Coming-Up over the same `queue_playback` snapshot, `media_type: 'film'`), covering series/single/boxset; the hero subtitle (the source's title) is caller-supplied — an opaque series/boxset id needs its own lookup, unlike home movies' derivable person/month slugs — and Shuffle/Repeat render disabled-but-visible (`hasSource` false) rather than always-enabled, for a standalone film with nothing to shuffle/repeat (TASK-493 row 21)
- `video-player-router.js` — persistent video-player view-router (FEAT-037) — 'video'/legacy-queue-mode ONLY since TASK-503 (the browse "Play Queue" entry; series/single moved onto `queue-playback-router.js` below)
...
- `queue-playback-router.js` — the shared view-router for the TASK-498 unified queue engine's own `queue_playback` snapshot (item_id-keyed now_playing, `queue`/`next`/`coming_up` lookahead lists), over `video-player-router.js`/`music-video-playback-router.js` — home movies (TASK-499) and now films (TASK-503, series/single/boxset) both read it; TASK-504/505 cut the remaining two media types over onto it the same way
- `music-video-playthrough.js` — music-video routing only now (BUG-485 retired its order/index/Shuffle-Repeat half): `entryMode` is the one shared video.html dispatch table screen-video-page.js reads (home-movie AND series/single modes route through it too, but TASK-499/503 moved their actual playback off the video engine onto the unified queue engine, `queue-playback-router.js` above — `queue` mode, the browse "Play Queue" tile, is the one mode still on the video engine) — plus `playlistTrackTarget`/`playlistQueueKey` (TASK-374/376/421) and `mvTransportVisibility` (TASK-407)
...
- `screen-queue-shell.js` — **THE** TV Queue View overlay, for every media type (TASK-515/FEAT-497): one controller replacing the per-cutover copies below, which differed only in which view module they imported. `config.media` is the `core/queue-shell-config.js` entry; `config.getSourceTitle()` supplies the hero's source line for a type whose source id is opaque, read fresh on every render (so a late-resolving title lands via `refreshSourceTitle()`); `config.onToggle` is the one device-local control. Nothing points at it until TASK-516/517
- `screen-home-movies-queue.js` — ⚠️ superseded by `screen-queue-shell.js` above; deleted by TASK-516. Home-movies Queue UX shell overlay (TASK-499/FEAT-497), sharing the same `#queue-overlay` DOM + `#btn-queue` trigger (mode-selected via `screen-video-page.js`'s `engineMode`); `config.onToggle` is its one device-local control (the hero's Play/Pause toggles the mounted `<video>` directly, never a server action)
- `screen-film-queue.js` — ⚠️ superseded by `screen-queue-shell.js` above; deleted by TASK-517. Film (series/single/boxset) Queue UX shell overlay (TASK-503/FEAT-497), the same shape as `screen-home-movies-queue.js` above; `config.getSourceTitle()` supplies the hero's subtitle (`screen-video-page.js`'s own `seriesTitle`, read live so a title that resolves after the overlay's first paint still lands via `refreshSourceTitle()`)
...
- `companion-queue-shell.js` — **THE** companion Queue page, for every media type (TASK-515/FEAT-497): the phone mirror of `screen-queue-shell.js`, replacing the per-cutover copies below. `initQueueShellPage({ media, loadSourceName, contextPage })` — `media` picks the `queue_playback` snapshots this page repaints on and the engine its POSTs go to; `loadSourceName` is the source-title lookup a type with an opaque source id needs, absent for one that derives its own. Renders the "‹ Now Playing › Queue" crumb into the page's `#queue-crumb` mount rather than the page hardcoding a bare back button. Nothing points at it until TASK-516/517
- `companion-home-movies-queue.js` — ⚠️ superseded by `companion-queue-shell.js` above; deleted by TASK-516. The home-movies Queue UX shell mirror (TASK-499/FEAT-497), the same shape as `companion-music-video-queue.js` above over `queue_playback` (filtered to `media_type: 'home-movie'`)
- `companion-film-queue.js` — ⚠️ superseded by `companion-queue-shell.js` above; deleted by TASK-517. The film Queue UX shell mirror (TASK-503/FEAT-497), the same shape as `companion-home-movies-queue.js` above (filtered to `media_type: 'film'`); resolves its own hero-subtitle source title (a series/boxset id, opaque unlike home movies' derivable slugs) via `loadSeries`, once per distinct `source_id`
