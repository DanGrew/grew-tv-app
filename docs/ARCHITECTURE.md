...
- `screen-film-queue.js` — film (series/single/boxset) Queue UX shell overlay (TASK-503/FEAT-497), the same shape as `screen-home-movies-queue.js` above; `config.getSourceTitle()` supplies the hero's subtitle (`screen-video-page.js`'s own `seriesTitle`, read live so a title that resolves after the overlay's first paint still lands via `refreshSourceTitle()`)
...
- `companion-film-queue.js` — the film Queue UX shell mirror (TASK-503/FEAT-497), the same shape as `companion-home-movies-queue.js` above (filtered to `media_type: 'film'`); resolves its own hero-subtitle source title (a series/boxset id, opaque unlike home movies' derivable slugs) via `loadSeries`, once per distinct `source_id`
