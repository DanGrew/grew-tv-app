...
- `music-video-queue-view.js` — music-video Queue View model + markup, with the every-row-editable bucketing music's own retired `queue-view.js` had (FEAT-418/TASK-420) against its own `music_video_playback` snapshot (TASK-419), poster art and no now-playing position. The last per-media-type Queue view left; TASK-505 retires it onto the shell
- `video-player-router.js` — persistent video-player view-router (FEAT-037) for the OLD video engine. ⚠️ UNREAD since TASK-517 moved its last consumer (the browse "Play Queue" entry) onto the unified engine — kept, like the engine itself, until the old engines are retired wholesale once music + music videos move too (TASK-504/505)
...
- `queue-playback-router.js` — the shared view-router for the TASK-498 unified queue engine's own `queue_playback` snapshot (item_id-keyed now_playing, `queue`/`next`/`coming_up` lookahead lists), over `video-player-router.js`/`music-video-playback-router.js` — home movies (TASK-499/516) and films (TASK-503, plus the browse "Play Queue" entry in TASK-517) both read it; TASK-504/505 cut the remaining two media types over onto it the same way. `queueCount` is browse's 🎬 pill count, moved here off the old engine's router in TASK-517
- `music-video-playthrough.js` — music-video routing only now (BUG-485 retired its order/index/Shuffle-Repeat half): `entryMode` is the one shared video.html dispatch table screen-video-page.js reads (home-movie AND series/single modes route through it too, but TASK-499/503 moved their actual playback off the video engine onto the unified queue engine, `queue-playback-router.js` above — and TASK-517 moved `queue` mode, the browse "Play Queue" tile, the last one left on the video engine) — plus `playlistTrackTarget`/`playlistQueueKey` (TASK-374/376/421) and `mvTransportVisibility` (TASK-407)
...
- `screen-video-queue.js` — the pre-FEAT-497 video Queue View overlay (FEAT-040), ⚠️ unreachable since TASK-517 (no `engineMode` resolves to `video` any more), kept until the old engines are retired. Music's own `screen-queue.js` went with TASK-504's cutover — the audio player mounts `screen-queue-shell.js` now
...
- `companion-video-queue.js` · `companion-music-video-queue.js` — the music-video Queue View mirror (FEAT-418/TASK-420); ships `mountScreenBar` from the start (TASK-417's gap, not repeated here). Music's own `companion-queue.js` went with TASK-504 — `companion/music-queue.html` mounts `companion-queue-shell.js`
- `companion-queue-shell.js` — **THE** companion Queue page, for every media type (TASK-515/FEAT-497): the phone mirror of `screen-queue-shell.js`, replacing the per-cutover copies below. `initQueueShellPage({ media, loadSourceName, contextPage })` — `media` picks the `queue_playback` snapshots this page repaints on and the engine its POSTs go to; `loadSourceName` is the source-title lookup a type with an opaque source id needs, absent for one that derives its own. Renders the "‹ Now Playing › Queue" crumb into the page's `#queue-crumb` mount rather than the page hardcoding a bare back button. Home movies (`home-movies-queue.html`, TASK-516) and films (`film-queue.html`, TASK-517, passing `loadSeriesTitle` for its opaque series/boxset id) run entirely on it; music/music videos follow in TASK-504/505
