import { filmQueueViewHtml } from '../../core/film-queue-view.js';
import { queueCrumbHtml } from '../../core/queue-crumb.js';

// TASK-503 (FEAT-497) Film Queue View overlay — the second media type on the
// docs/QUEUE-UX-SHELL.md shell (TASK-499 was the first, home movies; same
// shape). Hangs off the persistent video player (screen-video-page.js, the
// series/single 'film' engine mode): the <video> stays mounted and this
// layer draws the server `queue_playback` snapshot (TASK-498) — the hero +
// Queue/Next/Coming-Up tabs — on top. It owns no queue state: it renders the
// snapshot and turns each control into a queue engine action (play-item /
// remove-queue-entry / move-queue-entry / toggle-shuffle / toggle-repeat /
// next / previous), then repaints from the next snapshot the server pushes.
// Play/pause is the one device-local control (config.onToggle) — it toggles
// the already-mounted <video> directly, not a server action. The phone
// mirror is ui/screens/companion-film-queue.js.
//
// config.getSourceTitle() supplies the hero's subtitle (the series/boxset
// title, fetched by screen-video-page.js the same way its own breadcrumb
// resolves one — a lookup by source_id, not derivable from the engine
// snapshot alone) — read fresh on every render, so a title that resolves
// after the overlay's first paint (an async fetch racing the first snapshot)
...
export function setupFilmQueue(config) {
  ...
  var onAction = config.onAction;      // (action, payload) -> POST /api/queue/film/{action}
  ...
  var getSourceTitle = config.getSourceTitle; // () -> current hero subtitle
  var lastSnap = null;
  ...
  // buttons count — a disabled Shuffle/Repeat (no source to shuffle/repeat)
  // is excluded the same way a disabled queue-row action already is.
  function buildGrid() {
  ...
  function render(snap) {
    lastSnap = snap;
    body.innerHTML = filmQueueViewHtml(snap, getSourceTitle());
  ...
  // The hero subtitle resolves asynchronously (getSourceTitle's own fetch)
  // and may land after the overlay is already open — a plain re-render off
  // the last snapshot picks it up without waiting for the next server push.
