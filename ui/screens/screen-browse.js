...
var STATE = { server: null, cards: [], cw: [], cwPlaylists: [], progress: {}, labels: {}, profile: null, onSelect: null, onQueue: null, onCreatePlaylist: null };
...
function selectTab(tabId) {
  ...
  markActive(tabId);
  var rails = buildTabRails(tabId, STATE.cards, STATE.cw, STATE.labels, STATE.cwPlaylists);
...
// /api/browse cards + the /api/continue-watching rows + its `playlists` field
// (FEAT-044/TASK-285 — in-progress playlist tiles on the Continue Listening
// rail) + genreLabels, the active `person` (FEAT-033 — its authored name + glyph
// badge the bar), the select handler, and an optional initialTab to land on
// (else the first tab). The CW rows feed both the per-tab Continue Watching rail
// and the tiles' progress bars (via progressMapFromCW).
export function renderBrowse(server, cards, cwRows, cwPlaylists, labels, profile, person, onSelect, initialTab, onQueue, onCreatePlaylist) {
  STATE.server = server;
  STATE.cards = cards;
  STATE.cw = cwRows;
  STATE.cwPlaylists = cwPlaylists;
