...
runFlow({
  ...
  steps: [
    { name: '01-music', fn: async p => { await p.locator('#section-dock .dock-tab', { hasText: 'Music' }).first().click(); await p.locator('[data-create-playlist]').first().waitFor(); } },
    ...
    { name: '03-created', fn: async p => { await p.locator('#pl-name').fill(NAME); await p.locator('#btn-create').click(); await p.waitForURL(/browse\.html/, { timeout: 15000 }); await p.locator('#section-dock .dock-tab', { hasText: 'Music' }).first().click(); await rail(p, 'Playlists').click(); await p.locator('#txtgrid .ph-txt', { hasText: NAME }).first().waitFor(); } },
